...
**YAML Syntax Diagnostic** (YAML 语法诊断):
A user-facing explanation of why the editor text is not parseable YAML. It belongs before Word Schema Diagnostic because schema validation cannot run until the YAML parses. The top-level editor state may stay simple, such as "invalid", but the editor should mark the specific line where the user can act. Common syntax diagnostics should translate parser wording into practical guidance: bad indentation inside a block scalar should say that the text under a `|` field must stay indented; duplicate keys should mark the repeated key; unclosed quotes should mark the line with the quote; and malformed mappings should point to the nearest line whose indentation or colon usage likely broke the YAML. Parser messages may be kept internally for debugging, but user-facing hints should not expose phrases such as "multiline key may not be an implicit key" without explanation.
_Avoid_: raw parser error, generic invalid YAML

**Word Schema Diagnostic** (词条结构诊断):
A user-facing explanation of why parseable Word YAML cannot be saved under the current Word Schema Version. It is distinct from YAML Parse Status: parseable YAML may still have Word Schema Diagnostics. The top-level editor state may stay simple, such as "invalid", because non-technical users do not benefit from a full error console. The useful help belongs at the YAML location itself: invalid fields, malformed keys, missing required sections, and wrong values should be underlined or otherwise marked in the editor, with short natural-language guidance attached to the marked line or field. Missing fields should be marked at the nearest existing parent; for example, if `etymology.historical_origins.source_word.language` is missing and `source_word` exists, mark the `source_word` line and say that `language` is missing there. For malformed or unknown keys, high-confidence near matches should be called out directly. For example, if `historical_origins:abc` causes the expected `historical_origins` section to appear missing, the editor should mark `historical_origins:abc` first and explain that it may be a mistyped `historical_origins` key. Unknown fields that do not closely match a known field should simply be marked as unsupported by the current Word structure. Diagnostic wording should include a short repair direction, such as "这里缺少 language 字段", "这里应该是一组条目，用 - 开头", or "当前结构不支持这个字段"; it should not expose raw validator terms. The same diagnostic meaning should be available in the YAML Editor, manual repair dialog, Workset save result, and API responses; surfaces may compress the wording, but they should not collapse specific field errors into an unlocated generic message.
_Avoid_: error console, raw Zod issue, schema freshness

**Current Word Strictness** (当前词条严格度):
When saving a new or current Word, the YAML should not silently keep extra fields outside the current Word structure. Extra fields should become Word Schema Diagnostics so the YAML Editor can mark them in place. This is different from old Word maintenance, where non-current Content may be saved losslessly under the Editor Save Intent rules. If future extensibility is needed, the schema should add an explicit extension field such as notes or metadata rather than relying on object passthrough.
_Avoid_: silent passthrough, hidden extra fields

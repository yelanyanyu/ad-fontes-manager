...
- **AI 智能生成**：4 阶段流水线（结构化研究 → 创意富化 → 质量审核 → 自动修复），支持批量生成、SSE 实时进度、断点续传、暂停/恢复
- **AI 作业队列**：并发池配置、作业历史、工作集批量保存、限流与去重保护
- **词条管理**：搜索、排序、分页、多语言支持（英语 / 德语），4 种排序模式、跨页选择、一键清除
...
- **桌面模式**：Electron 桌面程序，自定义数据目录，版本号与 Copyright 显示，自定义应用图标
- **新手引导**：5 步交互式引导，覆盖完整工作流，支持重放
- **多厂商 AI 支持**：OpenAI 官方、硅基流动、DashScope 等第三方 OpenAI 兼容厂商，以及 Anthropic 格式模型
...
3. **质量审核**（auditing）— 评分并给出修改建议
4. **自动修复**（fixing）— 根据审核意见自动修复不达标字段（可选）

支持批量生成、SSE 实时进度、断点续传、暂停/恢复、自定义评分、字段局部重新生成。

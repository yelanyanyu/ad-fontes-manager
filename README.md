...
- **公告系统**：Header 右侧铃铛图标，自动拉取远程公告内容（JSON），未读红点徽章提示，支持本地缓存兜底
- **新手指引**：首次启动时渐进式引导（driver.js），5 步高亮各功能区块，完成后不再显示

### Phase 2: LLM 集成批量生成 YAML

- **AI 配置**：支持自定义供应商平台（OpenAI / Anthropic / DeepSeek / 第三方代理等），按流水线阶段分派不同模型以节省花费
- **智能搜索**：LLM 通过 tool call 调用搜索 + 网页抓取工具，限定词源网站域名（etymonline、dwds 等），按语言分组默认配置
- **生成 Pipeline**：两阶段生成（轻量模型做搜索+结构字段 → 重量模型做创意写作），格式自动修复，内容质量打分，低分字段自动重生成
- **批量工作流**：多单词并发处理，SSE 实时进度推送，审核结果表格 + 逐字段详情面板，一键批量保存到词库

> 详细计划见 [task_plan_phase2.md](./task_plan_phase2.md)（LLM 集成）和 [task_plan.md](./task_plan.md)（公告系统）。

...
项目是 monorepo 结构，根目录 `pnpm install` 会安装所有依赖（包括 Electron、Vue、Express 等）。`better-sqlite3` 原生模块的 ABI 切换由脚本手动控制（参见下方「原生模块（ABI 切换）」章节）。
...
pnpm run start:web      # 启动生产后端（NODE_ENV=production）
```
...
```bash
pnpm run native:node        # 为 Web/dev 准备 ABI 127
pnpm run native:electron    # 为 Electron 准备 ABI 140
```

日常开发不需要手动记 — `pnpm run dev:web` 和 `pnpm run build:desktop:win` 会自动调用对应脚本。

**桌面构建流程**：

```text
pnpm run build:desktop:win|mac
  ├── electron-vite build
  ├── pnpm run native:electron            # 切换到 Electron ABI 140
  ├── electron-builder                   # 打包
  └── finally: pnpm run native:node       # 恢复 Node ABI 127（无论构建成败）
```
...
| 现象                                     | 根因                                    | 处理                                                                                          |
| ---------------------------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------- |
| Web/dev 报 `ECONNRESET`                  | 根 `node_modules` 处于 Electron ABI 140 | `pnpm run native:node`                                                                        |
| 桌面启动后闪退                           | 打包的 `.node` 是 Node ABI 127          | 确保用 `pnpm run build:desktop:win` 而非直接 `electron-builder`                               |
...
```bash
pnpm run test           # Vitest 前端单元测试
pnpm run test-api       # API 接口测试（需要运行中的后端）
```

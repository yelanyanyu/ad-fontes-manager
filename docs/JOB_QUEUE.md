...
```bash
pnpm run dev:web        # 自动确保 Node ABI 127
pnpm run dev:desktop    # 自动确保 Electron ABI 140
pnpm run build:desktop:win  # 构建过程中自动切换 ABI
```

如果僵尸进程持有文件锁，重建脚本会自动终止它们。手动重建：

```bash
pnpm run rebuild:electron:native  # 强制重建 Electron ABI
pnpm run rebuild:node:native      # 强制重建 Node ABI
pnpm run native:node              # 自动检测 + 修复 Node ABI
```

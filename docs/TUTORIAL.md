...
其他可选厂商：OpenAI、Anthropic、DashScope（阿里云）、AIHubMix。每个厂商的获取方式类似。

### 3.2 填入 API Key

1. 点击左侧边栏的**齿轮图标**（Settings）
2. 点击左侧 **API** 标签
3. 在「AI 提供商」列表中，找到 **Silicon**
4. 在右侧面板中粘贴你的 API Key
5. 点击 **Test** 按钮——如果显示绿色成功提示，说明连接正常
...
2. 确认三个阶段的模型已分配：
   - **fast** → Silicon / deepseek-ai/DeepSeek-V3.2（负责查词源）
   - **balanced** → Silicon / deepseek-ai/DeepSeek-V3.2（负责生成中文创意内容）
   - **expert** → Silicon / deepseek-ai/DeepSeek-V3.2（负责审核评分）
3. Reasoning Effort（推理强度）：expert 阶段建议选 `high`

<!-- TODO: 截图 — Stages 配置页面 -->

> **使用其他厂商？** Anthropic 用户选 `deepseek-v4-pro[1m]` 等 Anthropic 格式模型。OpenAI 官方用户选 `gpt-5` 等。每个厂商可独立配置。
...
| 🔧 Fixing | 自动修复 | 根据建议修改（手动触发） | 30-120 秒 |
...
1. **审核评分**（overall_score）：满分 10 分，6 分及以上为合格
...
审核通过后，点击 **Fill Editor** 按钮，生成的 YAML 会填入左侧编辑器。
...
3. 如果词条已存在（同一个单词 + 同一种语言），系统会弹出**冲突检测**窗口，展示新旧版本差异
4. 选择「覆盖」或「取消」
...
- 检查硅基流动账户余额是否充足
- 在 Settings → API → Providers 中点击 Test 按钮确认连通性
...
- 如果你是自己手写 YAML，检查字段名是否拼写正确、层级缩进是否正确
- 如果是 AI 生成的内容，这通常意味着 LLM 输出有格式问题——尝试点击 Improve Text 重新生成

### 生成的叙事太像 AI 写的

- 检查 Settings → API → Stages → expert 阶段的 Reasoning Effort 是否设置为 high
...
3. 在新电脑的 Settings 中将数据目录指向复制后的位置
4. 重启应用

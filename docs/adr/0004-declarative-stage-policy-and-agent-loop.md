...
Implemented contract: each `PipelineStage` may declare `policy.execution`, `policy.output`, `policy.assembly`, and `policy.stopLoss`. Existing `searching`, `pondering`, `auditing`, and `fixing` Stage ids remain supported through a legacy policy adapter, while current English, German, Fix, and Audit-Fix Pipeline Definitions declare the policy explicitly.

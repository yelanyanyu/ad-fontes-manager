...
const { NonEmptyString, OptionalTrimmedString } = require('../common') as {
...
const stripAnkiTokenSyntax = (raw: string): string | null => {
  ...
  if (ANKI_FIELD_MODIFIERS.has(left)) {
    // {{hint:FieldName}} or {{clickable:FieldName}} â†?real field is on the right
    return unprefixed.slice(colonIdx + 1) || null;
  }
  // {{FieldName:filter}} â†?real field is on the left
  return left;
};

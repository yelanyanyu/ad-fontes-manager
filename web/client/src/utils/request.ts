...
const request: RequestClient = {
  ...
  post<T = any>(url: string, data?: unknown, config?: RequestConfig): Promise<T> {
    return withRateLimit(() => service.post(url, data, config as InternalRequestConfig)) as Promise<T>;
  },

  put<T = any>(url: string, data?: unknown, config?: RequestConfig): Promise<T> {
    return withRateLimit(() => service.put(url, data, config as InternalRequestConfig)) as Promise<T>;
  },

...
export interface WordSchemaInput {
...
const WordSchema = z
  .object({
    ...
    yield: requiredObject(
      {
        ...
        contextual_meaning: requiredObject(
          {
            en: requiredString('yield.contextual_meaning.en'),
    ...
    cognate_family: requiredObject(
      {
        cognates: nonEmptyArray('cognate_family.cognates').refine(rows => {
          return rows.every(item => {
            if (!isRecord(item)) return false;
            return isNonEmptyString(item.word) && isNonEmptyString(item.logic);
          });
    ...
    application: requiredObject(
      {
        selected_examples: nonEmptyArray('application.selected_examples').refine(rows => {
          return rows.every(item => {
            if (!isRecord(item)) return false;
            return (
              isNonEmptyString(item.type) &&
              isNonEmptyString(item.sentence) &&
              isNonEmptyString(item.translation_zh)
            );
          });
    ...
    nuance: requiredObject(
      {
        ...
        synonyms: nonEmptyArray('nuance.synonyms').refine(rows => {
          return rows.every(item => {
            if (!isRecord(item)) return false;
            return isNonEmptyString(item.word) && isNonEmptyString(item.meaning_zh);
          });

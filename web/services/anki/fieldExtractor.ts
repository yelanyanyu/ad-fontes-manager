...
function renderGermanEtymologyCard(etymData: Dict): string {
  ...
  const components = (morph.components as Array<{ element?: string; type?: string; de_meaning?: string }> | undefined) || [];
  const origins = (etymData.historical_origins as Dict | undefined) || {};
  ...
  if (phonology.pie_root || phonology.proto_germanic) {
    ...
    if (toText(phonology.grimm_step)) html += `<p style="${s.p} font-size: 14px;"><strong>Grimm:</strong> ${toText(phonology.grimm_step)}</p>`;
    if (toText(phonology.verner_law)) html += `<p style="${s.p} font-size: 14px;"><strong>Verner:</strong> ${toText(phonology.verner_law)}</p>`;
    ...
    if (toText(phonology.consonant_shift)) html += `<p style="${s.p} font-size: 14px;"><strong>Konsonantenverschiebung:</strong> ${toText(phonology.consonant_shift)}</p>`;
    html += `</div>`;
  }
  ...
  if (semantics.proto_meaning || semantics.semantic_shifts) {
    ...
    if (toText(semantics.proto_meaning)) html += `<p style="${s.p} font-size: 14px;"><strong>Urbedeutung:</strong> ${toText(semantics.proto_meaning)}</p>`;
    if (toText(semantics.semantic_shifts)) html += `<p style="${s.p} font-size: 14px;"><strong>Bedeutungsverschiebungen:</strong> ${toText(semantics.semantic_shifts)}</p>`;
    html += `</div>`;
  }
...
function renderNuanceCard(data: Dict): string {
  ...
  if (de && data.observations) {
    ...
    if (hasObs) {
      ...
      if (toText(obs.register)) html += `<p style="${s.p} font-size: 13px;"><strong>Register:</strong> ${toText(obs.register)}</p>`;
      if (toText(obs.false_friends)) html += `<p style="${s.p} font-size: 13px;"><strong>False Friends:</strong> ${toText(obs.false_friends)}</p>`;
      if (toText(obs.calque_status)) html += `<p style="${s.p} font-size: 13px;"><strong>Calque Status:</strong> ${toText(obs.calque_status)}</p>`;
      html += `</div></div>`;
    }
  }

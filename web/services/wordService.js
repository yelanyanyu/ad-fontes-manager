...
class WordService {
  ...
  _validateStrictYaml(data, wordLower) {
    ...
    if (yieldData) {
      ...
      if (
        this._isNonEmptyString(yieldData.lemma) &&
        yieldData.lemma.trim().toLowerCase() !== wordLower
      ) {
        errors.push('yield.lemma must match word');
      }
      if (!this._isNonEmptyString(yieldData.syllabification))
        errors.push('yield.syllabification is required');
      if (!this._isNonEmptyString(yieldData.user_context_sentence))
        errors.push('yield.user_context_sentence is required');
      if (!this._isNonEmptyString(yieldData.part_of_speech))
        errors.push('yield.part_of_speech is required');
      if (!this._isObject(yieldData.contextual_meaning))
        errors.push('yield.contextual_meaning is required');
      if (this._isObject(yieldData.contextual_meaning)) {
        if (!this._isNonEmptyString(yieldData.contextual_meaning.en))
          errors.push('yield.contextual_meaning.en is required');
        if (!this._isNonEmptyString(yieldData.contextual_meaning.zh))
          errors.push('yield.contextual_meaning.zh is required');
      }
      if (
        !Array.isArray(yieldData.other_common_meanings) ||
        yieldData.other_common_meanings.length === 0
      ) {
    ...
    if (etymology) {
      ...
      if (this._isObject(roots)) {
        if (!this._isNonEmptyString(roots.prefix))
          errors.push('etymology.root_and_affixes.prefix is required');
        if (!this._isNonEmptyString(roots.root))
          errors.push('etymology.root_and_affixes.root is required');
        if (!this._isNonEmptyString(roots.suffix))
          errors.push('etymology.root_and_affixes.suffix is required');
        if (!this._isNonEmptyString(roots.structure_analysis))
          errors.push('etymology.root_and_affixes.structure_analysis is required');
      }
      ...
      if (this._isObject(origins)) {
        if (!this._isNonEmptyString(origins.history_myth))
          errors.push('etymology.historical_origins.history_myth is required');
        if (!this._isNonEmptyString(origins.source_word))
          errors.push('etymology.historical_origins.source_word is required');
        if (!this._isNonEmptyString(origins.pie_root))
          errors.push('etymology.historical_origins.pie_root is required');
      }
      if (!this._isNonEmptyString(etymology.visual_imagery_zh))
        errors.push('etymology.visual_imagery_zh is required');
      if (!this._isNonEmptyString(etymology.meaning_evolution_zh))
        errors.push('etymology.meaning_evolution_zh is required');
    }
    ...
    if (this._isObject(cognateFamily)) {
      ...
      if (!Array.isArray(cognates) || cognates.length === 0) {
        ...
        const invalid = cognates.some(
          c =>
            !this._isObject(c) ||
            !this._isNonEmptyString(c.word) ||
            !this._isNonEmptyString(c.logic)
        );
    ...
    if (this._isObject(application)) {
      ...
      if (!Array.isArray(examples) || examples.length === 0) {
        ...
        const invalid = examples.some(
          e =>
            !this._isObject(e) ||
            !this._isNonEmptyString(e.type) ||
            !this._isNonEmptyString(e.sentence) ||
            !this._isNonEmptyString(e.translation_zh)
        );
        if (invalid)
          errors.push(
    ...
    if (this._isObject(nuance)) {
      if (!this._isNonEmptyString(nuance.image_differentiation_zh))
        errors.push('nuance.image_differentiation_zh is required');
      const synonyms = nuance.synonyms;
      if (!Array.isArray(synonyms) || synonyms.length === 0) {
        errors.push('nuance.synonyms must be a non-empty array');
      } else {
        const invalid = synonyms.some(
          s =>
            !this._isObject(s) ||
            !this._isNonEmptyString(s.word) ||
            !this._isNonEmptyString(s.meaning_zh)
        );
  ...
  async addWord(req, wordText, yamlStr) {
    ...
    try {
      ...
      const existingRes = await client.query(
        'SELECT id, lemma FROM words WHERE lower(lemma) = $1',
        [wordLower]
      );
      if (existingRes.rows.length > 0) {
        ...
        return {
          status: 'duplicate',
          lemma: existingRes.rows[0].lemma,
  ...
  async saveWord(req, yamlStr, forceUpdate) {
    ...
    try {
      ...
      // Check Existence
      const existingRes = await client.query('SELECT * FROM words WHERE lower(lemma) = $1', [
        lemma,
      ]);
